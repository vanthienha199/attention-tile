...
\TLV
   |scorer
      ...
      @2
         ...
         $q_complete  = >>1$qfill[3];
         $key_in_range = ~>>1$key_idx[6];
         $do_accumulate = >>1$is_stream_k && $key_in_range;
         $dim_complete  = &>>1$dim[2:0];
         $key_done      = $do_accumulate && $dim_complete;
         $new_is_better = \$signed($sat_sum) > \$signed(>>1$best[15:0]);

         $wr_idx[2:0] = $q_complete ? 3'h0 : >>1$qfill[2:0];
         $write_en[7:0] = >>1$is_load_q ? (8'h1 << $wr_idx) : 8'h0;

         $completing_q = >>1$is_load_q && (>>1$qfill[3:0] == 4'h7);

         $qfill_nxt[3:0] =
            >>1$is_load_q ?
               ($q_complete ? 4'h1 : (>>1$qfill + 4'h1)) :
            (~>>1$cmd[1] || >>1$is_stream_k) ?
               ($q_complete ? >>1$qfill : 4'h0) :
            >>1$qfill;

         $reset_dim_or_keydone = ~>>1$cmd[1] || $key_done;
         $dim_nxt[2:0] = $reset_dim_or_keydone ? 3'h0 : ($do_accumulate ? (>>1$dim + 3'h1) : >>1$dim);
         $acc_nxt[15:0] = $reset_dim_or_keydone ? 16'h0000 : ($do_accumulate ? $sat_sum : >>1$acc);
         ...
         <<1$q0[7:0]         = *reset ? 8'h00 : ($write_en[0] ? >>1$data : >>1$q0);
         <<1$q1[7:0]         = *reset ? 8'h00 : ($write_en[1] ? >>1$data : >>1$q1);
         <<1$q2[7:0]         = *reset ? 8'h00 : ($write_en[2] ? >>1$data : >>1$q2);
         <<1$q3[7:0]         = *reset ? 8'h00 : ($write_en[3] ? >>1$data : >>1$q3);
         <<1$q4[7:0]         = *reset ? 8'h00 : ($write_en[4] ? >>1$data : >>1$q4);
         <<1$q5[7:0]         = *reset ? 8'h00 : ($write_en[5] ? >>1$data : >>1$q5);
         <<1$q6[7:0]         = *reset ? 8'h00 : ($write_en[6] ? >>1$data : >>1$q6);
         <<1$q7[7:0]         = *reset ? 8'h00 : ($write_en[7] ? >>1$data : >>1$q7);
