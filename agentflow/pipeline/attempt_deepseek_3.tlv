...
\TLV
   |scorer
      @0
         ...
         // Pipeline stage: multiply in @0, use in @1
         $prod_p[15:0] = \$signed($qsel[7:0]) * \$signed($data[7:0]);

      @1
         // Use delayed product from @0 (implicit register)
         $prod[15:0] = $prod_p;
         ...
         // State updates (all at @1, as before)
